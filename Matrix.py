...
class Matrix:
    def __init__(self, villages: JSInstance, centres: JSInstance):
        ...
        df_centre = df_centre.astype(
            {'Latitude': 'float', 'Longitude': 'float'})

        self._rows = len(df_ville.index)
        self._cols = len(df_centre.index)

        self._matrix = [[0 for _ in range(self._cols)]
                        for _ in range(self._rows)]

        self.fill_matrix(df_ville=df_ville, df_centre=df_centre)
    ...
    def fill_matrix(self, df_ville: pd.DataFrame, df_centre: pd.DataFrame):
