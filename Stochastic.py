...
class Stochastic(object):
    ...
    def __init__(self, centers, villages, detsolver, beta):
        self._model = pulp.LpProblem("Drone attribution", pulp.LpMaximize)
        self.center = centers
        self.village = villages
        self.drone = np.arange(1,16)
        self._y_init = detsolver.get_solution().get_open()
        self._z_init = detsolver.get_solution().get_drone_center()
        self.beta = beta
    ...
    def create_constraints(self):
        ...
        for i in self.center.get_all_ids():
            self._model += self._y[i] == self._y_init.get((i), 0)
